...
public abstract class AbstractLoadBalance implements LoadBalance {
    ...
    int getWeight(Invoker<?> invoker, Invocation invocation) {
        ...
        if (REGISTRY_SERVICE_REFERENCE_PATH.equals(url.getServiceInterface())) {
            ...
            weight = url.getMethodParameter(invocation.getMethodName(), WEIGHT_KEY, DEFAULT_WEIGHT); // 从 url 中获取权重 weight 配置值
            if (weight > 0) {
                ...
                if (timestamp > 0L) {
                    ...
                    if (uptime > 0 && uptime < warmup) {
                        weight = calculateWarmupWeight((int)uptime, warmup, weight); // 如果服务运行时间小于预热时间，则重新计算服务权重，即降权
                    }
                }
            }
        }
