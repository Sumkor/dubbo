...
public class MockClusterInvoker<T> implements ClusterInvoker<T> {
    ...
    public Result invoke(Invocation invocation) throws RpcException {
        ...
        String value = getUrl().getMethodParameter(invocation.getMethodName(), MOCK_KEY, Boolean.FALSE.toString()).trim(); // 获取 mock 配置值
        if (value.length() == 0 || "false".equalsIgnoreCase(value)) {
            //no mock // 无 mock 逻辑，直接调用其他 Invoker 对象的 invoke 方法，比如 FailoverClusterInvoker
            ...
            try {
                ...
                if(result.getException() != null && result.getException() instanceof RpcException){
                    ...
                    if(rpcException.isBiz()){
                        ...
                        result = doMockInvoke(invocation, rpcException); // 调用失败，执行 mock 逻辑
                    }
                }
